	            		...
	            		@foreach($theloai as $tl)
	            			@if(count($tl->loaitin) > 0)
							    <div class="row-item row">
				                	<h3>
				                		<a href="home">{{$tl->Ten}}</a> |
				                		@foreach($tl->loaitin as $lt)
				                			<small><a href="news_category/{{$lt->id}}/{{$lt->TenKhongDau}}.html"><i>{{$lt->Ten}}</i></a>/</small>
				                		@endforeach
				                	</h3>
				                	<?php
				                	...
				                	<div class="col-md-8 border-right">
				                		<div class="col-md-5">
					                        <a href="news/{{$firstNews->id}}/{{$firstNews->TieuDeKhongDau}}.html">
					                    ...
					                    <div class="col-md-7">
					                        ...
					                        <a class="btn btn-primary" href="news/{{$firstNews->id}}/{{$firstNews->TieuDeKhongDau}}.html">Đọc tin <span class="glyphicon glyphicon-chevron-right"></span></a>
										</div>
				                	</div>

									<div class="col-md-4">
										@foreach($data as $otherNews)
											<a href="news/{{$otherNews->id}}/{{$otherNews->TieuDeKhongDau}}.html">
												<h4>
